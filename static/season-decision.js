(function () {
  ...
  function mountCard() {
    ...
    var latestPatch = registry.latestReviewedPatch || {};
    var directPatch = (registry.heroPatchRecords || {})[slug(item.name)];
    if (!job) return;
    ...
    var patchStatus = directPatch
      ? 'Direct official record in Version ' + directPatch.patchVersion
      : 'No direct hero change recorded in the reviewed Version ' + safe(latestPatch.version || 'source') + ' notes';
    var patchDetail = directPatch
      ? safe(directPatch.summary) + ' Source date: ' + safe(directPatch.sourceDate) + '; reviewed: ' + safe(directPatch.reviewedAt) + '.'
      : 'The current source review covers Version ' + safe(latestPatch.version || 'unconfirmed') + ' (source date: ' + safe(latestPatch.sourceDate || 'unconfirmed') + '; reviewed: ' + safe(latestPatch.reviewedAt || 'unconfirmed') + '). This is not evidence that the hero is unchanged in every live system or stronger/weaker than another hero.';
    var patchRecord = directPatch || latestPatch;
    var patchLinks = [];
    if (patchRecord.guideUrl) patchLinks.push('<a href="' + safe(patchRecord.guideUrl) + '">Read the patch explanation</a>');
    if (patchRecord.sourceUrl) patchLinks.push('<a href="' + safe(patchRecord.sourceUrl) + '" target="_blank" rel="noopener noreferrer">Read the official patch source</a>');
    var patchLink = patchLinks.length ? ' ' + patchLinks.join(' · ') + '.' : '';
    card.innerHTML = '<h2>Season decision card</h2>' +
      '<p class="decision-label">OFFICIAL PROFILE + EDITORIAL FRAMEWORK</p>' +
      '<div class="decision-grid"><div><b>Current role</b><span>' + safe(role) + '</span></div><div><b>Role source</b><a href="' + safe(item.sourceUrl || registry.sources.officialRoster.url) + '" target="_blank" rel="noopener noreferrer">Official profile</a></div><div><b>Source date</b><span>' + safe(item.sourceDate || registry.sources.officialRoster.reviewedAt) + '</span></div><div><b>Registry review</b><span>' + safe(item.reviewedAt || registry.reviewedAt) + '</span></div><div><b>Current version status</b><span>' + patchStatus + '</span></div></div>' +
      '<p class="decision-status"><b>Patch review:</b> ' + patchDetail + patchLink + '</p>' +
      ...
      '<p class="decision-status"><b>Decision boundary:</b> ' + safe(registry.currentStatus) + ' <a href="/season/">Review the season hub</a> · <a href="/season/data-sources">How this evidence is evaluated</a>.</p>' +
